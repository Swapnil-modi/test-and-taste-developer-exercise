...
namespace Test_Taste_Console_Application.Domain.Services
{
    ...
    public class ScreenOutputService : IOutputService
    {
        ...
        public void OutputAllPlanetsAndTheirMoonsToConsole()
        {
            ...
            DisplayCompletedWirtingStatus("AllPlanetsAndTheirMoons");

        }

        public void OutputAllMoonsAndTheirMassToConsole()
        {
            DisplayLoadingStatus("AllMoonsAndTheirMass");
            ...
            DisplayWritingStatus("AllMoonsAndTheirMass");
            if (!moons.Any())
            {
            ...
            ConsoleWriter.CreateLine(columnSizesForMoons);
            ConsoleWriter.CreateEmptyLines(2);
            DisplayCompletedWirtingStatus("AllMoonsAndTheirMass");
            /*
        ...
        {
            DisplayLoadingStatus("AllPlanetsAndTheirAverageMoonGravity");
            //The function works the same way as the PrintAllPlanetsAndTheirMoonsToConsole function. You can find more comments there.
            var planets = _planetService.GetAllPlanets().ToArray();
            DisplayWritingStatus("AllPlanetsAndTheirAverageMoonGravity");
            if (!planets.Any())
            {
                Console.WriteLine(OutputString.NoMoonsFound);
                return;
            ...
            ConsoleWriter.CreateEmptyLines(2);
            DisplayCompletedWirtingStatus("AllPlanetsAndTheirAverageMoonGravity");
            /*
                --------------------+--------------------------------------------------
        ...
        private void DisplayLoadingStatus(string dataType)
        {
            Console.WriteLine($"Loading {dataType} ....");
        }
        private void DisplayWritingStatus(string dataTypes)
        {
            Console.WriteLine($"Writing {dataTypes} ....");
        }
        private void DisplayCompletedWirtingStatus(string dataTypes)
        {
            Console.WriteLine($"Completed Writing {dataTypes} ....");
        }
    }
}
